...
针对边缘计算中的资源管理问题，当前研究主要关注于移动边缘计算网络中的资源管理，考虑了MEC服务器资源的实际限制。研究重点包括单服务器和多服务器情境下的集中和分布式资源管理算法。其中，一些研究提出了半分布式方法，如启发式卸载算法（HODA），在复杂性可接受的情况下实现了卓越的系统效用[^b13]。其他研究采用集中式深度强化学习（DRL）或基于多智能体策略的DRL模型，但面临着行动空间激增的挑战和增加了计算能力较弱设备的复杂性[^b16][^b7]。
...
随意卸载任务可能引发边缘服务器之间的工作负载不平衡，这种不平衡可能导致服务器收入减少，同时用户的效用也受到损害。因此，对于优化服务质量和资源效率而言，边缘服务器之间的任务调度显得尤为关键。通过有效的任务调度，可以协调和平衡边缘服务器的工作负载，从而提高服务的质量和资源的利用效率。这一过程对于确保边缘计算网络的正常运行和提供稳定可靠的服务至关重要。

我们的目标是通过研究基于任务需求和预算的任务调度策略，实现对任务的有效管理。这包括将具有不同特性的任务从车辆卸载到合适的节点，以实现更加灵活和高效的资源分配。通过这一过程，我们旨在提高用户满意度，增加边缘服务器的收入，并最大化资源的利用率。这一系列目标的实现将有助于优化整体的服务质量，提高系统的经济效益，以适应动态且多变的物联网车辆环境。
