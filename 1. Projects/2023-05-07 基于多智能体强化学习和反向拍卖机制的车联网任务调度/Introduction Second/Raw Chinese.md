...
MEC的优势:低延迟,高带宽,实时无线网络信息,位置感知 [^e7][^e8]。

在移动边缘服务器和任务调度领域，不断涌现的技术和算法为面对高用户密度和资源限制的挑战提供了新的解决方案。

任务卸载算法的设计方面，面向不同的优化目标，如最小化延迟、最大化计算和通信速率, 节能减排等, 自适应算法[^c5][^c8]、契约理论建模[^c9]、深度强化学习[^c10]等多种方法被提出。

在车辆边缘计算（VEC）中，任务调度成为一个关键问题，其挑战在于地理差异导致边缘服务器负载不平衡。已有的调度算法着眼于能效、蚁群优化和计算密集型应用等不同方向。同时，对RSU的能源效率策略的研究涵盖了休眠策略和联合小区关联等方法，以最大程度降低能耗。

移动边缘计算（MEC）作为下一代5G网络的关键技术，在车联网（IoV）中发挥着重要作用。其优势包括低延迟、高带宽、实时无线网络信息和位置感知。基于MEC的IoV推动了计算任务从移动车辆卸载到网络边缘节点，以满足超低延迟需求。在物联网中，设备可以将计算任务卸载到MEC服务器，提高任务处理速度并节省设备能量。

然而，在IoV任务调度领域，仍然存在着一些挑战。车辆的动态性、波动的网络条件、任务的及时性和复杂性以及异构资源的限制都给任务调度带来了复杂性。为了应对这些挑战，研究人员采用了模糊逻辑、机器学习等常见方法，同时也提出了创新性的多主体强化学习与反拍卖机制相结合的方法，以实现更灵活、智能、协同的任务调度策略。

总体而言，移动边缘计算和车联网领域的研究和创新持续推动着技术的进步，为解决实际问题提供了多样化的解决途径。未来的发展将进一步关注系统的稳定性、适应性和性能优化，以更好地支持移动用户的需求。



**任务卸载算法设计:**
自适应算法专注于最小化平均响应时间。契约理论建模提出了一种目标是最小化网络延迟的算法。深度强化学习追求最大化计算和通信速率。能效资源分配算法[^c11]和最小化能耗和延迟的加权和算法[^c12]也在讨论中。

**VEC中现有的任务调度算法:**
能效调度算法[^c7]和基于蚁群优化的算法[^c13]提出了解决方案。一些算法考虑部分卸载调度和功率分配[^c14]，而另一些则面向计算密集型和时间敏感应用[^c15]。

**RSU的能源效率策略:**
RSU采用在低任务请求时段转入休眠状态的策略，以节省能量[^c16][^c17][^c18][^c19]。一些算法通过实施基站的休眠策略来最大化能源效率[^c20,21]，同时其他算法通过联合小区关联和开关方案最小化能耗[^c22]。另外，存在面向用户的能量感知移动管理方案[^c23]。

**MEC及其认证介绍:**
移动边缘计算（MEC）通过在移动用户附近部署服务器，有效降低服务交付和网络运营的延迟成本。MEC被欧洲5G基础设施公私合作伙伴关系（PPP）认证为下一代5G网络的关键技术。



**基于MEC的车联网（IoV）:**
MEC促进了从移动车辆卸载计算任务到网络边缘节点，以满足IoV的超低延迟需求 [^e9]。它将云服务下沉到无线接入网络的边缘，为行驶中的车辆提供计算服务 [^e10]。
...
为了应对这些挑战，研究人员和工程师采用了多种常见方法。其中一种是使用模糊逻辑，然而，这种方法需要预定义模糊规则的公式，受制于静态专业知识，可能无法很好地适应动态环境。另一种方法是机器学习，但由于数据集的陈旧或普遍性不足，调整任务调度策略可能会面临一些问题。
...
- Dai等人[^f25] 开发了一种联合最优的VEC服务器选择和卸载算法，以最大化系统效用。
...
大量文献集中在设计在不同性能要求下的最佳策略 [^f18][^f19][^f20][^f21]。为了实现物联网网络的长期能效，[^f18] 提出了一种高效的边缘计算基础设施。针对随机任务到达、无线信道变化、拥塞的空中接口和禁止性反馈，[^f19] 生成了对网络知识陈旧具有容忍性的渐近最优调度，减轻了对反馈的严格要求。此外，[^f20] 和 [^f21] 中分别提出了MEC的最优调度解决方案和能效资源分配策略。

在车辆在物联网系统中作为用户设备的重要角色下，研究人员广泛探索了车辆边缘计算网络的结构和资源分配方法 [^f22][^f23][^f24][^f25][^f26]。具体地，C. Wang等人提出了一种可扩展的SDN启用的MEC架构，以减少总体延迟并卸载骨干网络的流量负载 [^f22]。为了解决计算卸载的延迟和传输成本，[^f23] 提出了一种面向车辆网络的基于云的MEC卸载框架。在 [^f24] 中，提出了一种基于MEC辅助的车辆卸载模式的目标服务器选择策略，旨在提高在车辆数据传输失败的情况下任务卸载的可靠性。在 [^f25]、[^f26] 中提出了新颖的基于MEC的车辆网络，为不同场景精心设计了计算卸载策略。

然而，在现有文献中，车辆主要充当MEC网络中的用户，可能导致由于庞大数量的用户设备的服务请求而产生“服务空白”。因此，本文着重设计了一种车辆边缘计算（VEC）网络，使车辆能够在传统边缘服务器之外提供计算服务。传统的边缘服务器连接到固定位置，而提出的架构旨在扩展计算服务范围，增强移动边缘计算网络的灵活性。
...
在车辆边缘计算中，通过任务卸载到边缘服务器的道路边单元（RSU）来节省能耗并加速计算被认为是至关重要的[^c7]。在VEC中的现有工作主要集中在任务卸载算法设计上，以最小化响应时间或能耗。
