...
在物联网车辆（IoV）任务调度领域，面临着动态性、网络不稳定性和异构资源限制等挑战。车辆位置、状态和可用性的变化增加了调度复杂性，而波动的网络条件可能影响任务调度。任务的及时性和复杂性加大了调度难度，需要在有限时间内完成复杂任务。异构资源的限制意味着需要有效地分配不同任务以最大程度利用可用资源。随意卸载任务可能导致边缘服务器工作负载不平衡，影响服务器收入和用户效用。因此，边缘服务器的任务调度对于优化服务质量和资源效率至关重要。通过基于任务特征和预算的调度策略，我们的研究旨在实现对任务的有效管理，提高用户效用、增加边缘服务器收入，并最大化资源利用率，以适应动态且多变的物联网车辆环境。这将有助于优化整体服务质量，提高系统经济效益。

在MEC车联网中，车辆的高度移动性和时变性增加了多维资源管理的计算复杂性，需要更智能、实时的资源分配方法来解决这一挑战。然而，现有的纯优化方法在处理延迟敏感应用和计算复杂性较大的情况时存在困难。这些方法通常忽视了车辆和服务器之间的连通性，导致结果过于理想且难以应用于实际场景。连通性的考虑对任务调度和数据传输至关重要，因此基于DRL（深度强化学习）的解决方案可能面临在复杂环境中不明确的行动空间激增的问题，需要更好地定义和处理决策问题。长期资源平衡的考虑在现有方法中往往被忽略，但在车联网中，这是保持网络健康运行和提高整体效率至关重要的因素。未来的研究和解决方案需要更全面地考虑长期资源平衡的因素，以实现更好的负载均衡。
