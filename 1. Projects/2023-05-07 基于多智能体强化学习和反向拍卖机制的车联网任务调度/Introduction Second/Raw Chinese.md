...
移动边缘计算（MEC）作为下一代网络的关键技术，在车联网（IoV）中发挥着重要作用。其优势包括低延迟、高带宽、实时无线网络信息和位置感知 [^e7]。基于MEC的IoV推动了计算任务从移动车辆卸载到网络边缘节点，以满足超低延迟需求,可以将计算任务卸载到MEC服务器，提高任务处理速度并节省设备能量[^e8]。

在移动边缘服务器和任务调度领域，不断涌现的技术和算法为面对高用户密度和资源限制的挑战提供了新的解决方案。任务卸载算法的设计方面，面向不同的优化目标，如最小化延迟、最大化计算和通信速率、节能减排等，自适应算法、契约理论建模、深度强化学习等多种方法被提出 [^c5][^c8].

VEC中现有的任务调度算法涵盖了多种方向，包括能效调度算法、基于蚁群优化的算法、部分卸载调度和功率分配、面向计算密集型和时间敏感应用的算法 [^c7][^c13][^c14][^c15].
...
在MEC的认证方面，它被欧洲5G基础设施公私合作伙伴关系（PPP）认证为下一代5G网络的关键技术 [^e9].

基于MEC的车联网（IoV）推动了计算任务从移动车辆卸载到网络边缘节点，以满足IoV的超低延迟需求 [^e9]. 它将云服务下沉到无线接入网络的边缘，为行驶中的车辆提供计算服务 [^e10].

在基于MEC的物联网中，设备可以将计算任务卸载到MEC服务器，提高任务处理速度并节省设备能量 [^55][^56][^57].

未来的发展将进一步关注系统的稳定性、适应性和性能优化，以更好地支持移动用户的需求 [^e9].
...
传统方法处理MEC中的任务调度问题包括凸逼近[^61]，[^62]，博弈论[^64]，[^65]和元启发式算法[^60]，[^66]。然而，这些方法面临挑战，如指数增长的搜索空间和沉重的计算负担，尤其是在大规模场景中。

举例来说，Dai等人[^f25] 开发了一种联合最优的VEC服务器选择和卸载算法，以最大化系统效用。同时，Zhou等人[^ewo] 提出了一种基于交替方向乘子法的能效资源分配算法。
