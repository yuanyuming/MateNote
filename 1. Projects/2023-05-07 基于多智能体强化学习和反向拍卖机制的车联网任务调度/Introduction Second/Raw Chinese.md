...
MEC技术在车联网（IoV）中发挥着关键作用，其优势包括低延迟、高带宽、实时无线网络信息和位置感知[^e7][^e10]。在基于MEC的IoV中，计算任务从移动车辆卸载到网络边缘节点，以满足对超低延迟的需求，通过卸载到MEC服务器提高任务处理速度并节省设备能量[^e8][^55][^56][^57]。

此外，MEC技术不仅在车联网中推动了计算任务的下沉，将云服务提供至无线接入网络的边缘，为行驶中的车辆提供计算服务
未来的发展将更加关注系统的稳定性、适应性和性能优化，以更好地满足移动用户的需求[^e9]。

在移动边缘服务器和任务调度领域，针对高用户密度和资源限制等挑战，不断涌现的技术和算法为创新的解决方案提供了可能。在任务卸载算法设计方面，各种方法如自适应算法、契约理论建模和深度强化学习等被提出，以满足不同的优化目标（如最小化延迟、最大化计算和通信速率、节能减排等）[^c5][^c8]。传统方法处理移动边缘计算（MEC）中的任务调度问题，涵盖了凸逼近[^61]，[^62]，博弈论[^64]，[^65]和元启发式算法[^60]，[^66]等，但在大规模场景中面临挑战，如指数增长的搜索空间和沉重的计算负担。VEC中现有的任务调度算法涵盖了多种方向，如能效调度算法、基于蚁群优化的算法、部分卸载调度和功率分配、面向计算密集型和时间敏感应用的算法[^c7][^c13][^c14][^c15]。

当前针对边缘计算中的资源管理问题的研究主要集中在移动边缘计算网络中，考虑了MEC服务器资源的实际限制。研究关注点包括单服务器和多服务器情境下的集中和分布式资源管理算法。一些研究提出了半分布式方法，例如启发式卸载算法（HODA），在复杂性可接受的情况下取得了卓越的系统效用[^b13]。另一方面，其他研究采用集中式深度强化学习（DRL）或基于多智能体策略的DRL模型，但面临着行动空间激增的挑战和增加了计算能力较弱设备的复杂性[^b16][^b7]。

为了克服传统方法的局限性，一些创新性的方法被提出，其中之一是将多主体强化学习与反拍卖机制相结合。这一创新的目标在于实现最佳资源匹配，通过促进车辆之间的合作，最小化整体成本。这种方法的优势在于其能够更灵活地适应动态环境，并提供更智能、协同的任务调度策略。

利用深度神经网络（DNNs）的强大功能[^68]，[^69],[^71]，深度强化学习方法在处理具有连续动作空间的动态优化问题方面显示出潜力。强化学习还被应用于车辆云中的长期资源供应，以解决资源的动态需求和严格的服务质量（QoS）要求[^f27]。
...
当前在基于MEC的车联网领域，学术界的研究主要聚焦于架构设计、任务卸载和资源管理方案。其中，决定何时、是否以及卸载多少计算任务是主要技术挑战之一。在车辆边缘计算（VEC）中，任务调度成为关键问题，其挑战在于地理差异导致边缘服务器负载不平衡。已有调度算法关注能效、蚁群优化和计算密集型应用等方向。对RSU的能源效率策略的研究包括休眠策略和联合小区关联等方法，以最大程度降低能耗 [^c7]。为了应对挑战，研究人员采用了多种方法，其中包括模糊逻辑和机器学习。然而，现有文献中车辆主要充当MEC网络中的用户，可能导致“服务空白”问题。因此，本文专注于设计一种车辆边缘计算（VEC）网络，使车辆能够在传统边缘服务器之外提供计算服务，从而扩展计算服务范围，增强移动边缘计算网络的灵活性。

在物联网车辆（IoV）任务调度领域，面临着一系列显著挑战。首先，车辆的动态性增加了任务调度的复杂性，因为车辆的位置、状态和可用性不断变化。其次，波动的网络条件也带来了问题，由于网络连接的不稳定性，任务调度可能受到影响。任务的及时性和复杂性进一步提高了调度的难度，需要在有限时间内完成复杂任务。异构资源的限制意味着不同车辆具有不同的性能和能力，有效地分配任务以最大程度利用可用资源变得至关重要。

在这一环境下，随意卸载任务可能导致边缘服务器之间的工作负载不平衡，进而影响服务器收入和用户效用。因此，边缘服务器之间的任务调度对于优化服务质量和资源效率至关重要。通过有效的任务调度，可以协调和平衡边缘服务器的工作负载，提高服务质量和资源利用效率。这一过程对于确保边缘计算网络的正常运行和提供稳定可靠的服务至关重要。

我们的研究目标是通过基于任务需求和预算的调度策略，实现对任务的有效管理。这涉及将具有不同特性的任务从车辆卸载到适当的节点，以实现更加灵活和高效的资源分配。通过这一过程，我们旨在提高用户满意度、增加边缘服务器收入，并最大化资源利用率。这一系列目标的实现将有助于优化整体服务质量，提高系统经济效益，以适应动态且多变的物联网车辆环境。

在任务管理方面，我们的方法包括管理频谱、计算和存储资源，同时对用户进行任务卸载决策（参考文献[^a15^], [^a16^], [^a17^]）。通过这一综合的管理策略，我们旨在最大程度地优化整个系统的性能。相较于传统方法，我们采用了联合优化任务卸载的方式，以提高任务调度的效率（参考文献[^a10^], [^a15^]）。而大多数现有方法仅考虑了一到两个维度的资源，这使得它们难以直接支持高纬度资源需求，尤其是对于复杂的计算任务而言（参考文献[^a7^]）。另一方面，现有研究往往侧重于低移动性用户的多维资源管理，而在高度移动且动态的车联网环境下，这种方法可能受到限制（参考文献[^a18^], [^a19^]）。因此，我们的任务管理策略致力于克服这些限制，以更全面、灵活地满足多维度资源需求的计算任务。

在MEC车联网中，车辆的高度移动性和资源需求的时变性增加了多维资源管理的计算复杂性。这种高度动态的环境要求更为灵活和智能的资源管理策略。然而，纯优化方法在面对延迟敏感应用且计算复杂性较大的情况时变得不可行。因此，为了适应这一挑战，需要寻求更智能、实时的资源分配方法。

另一方面，现有方法往往忽视了边缘设备和服务器之间的地理近距离，从而影响了流量的分布。这种忽视可能导致网络性能的下降，因为地理位置的考虑对于任务调度和数据传输至关重要。在此背景下，基于DRL（深度强化学习）的解决方案可能面临不明确的行动空间激增，需要更好地定义和处理复杂的环境中的决策问题。

最后，缺乏对长期资源平衡的考虑可能成为阻碍网络资源利用进一步提高的因素。在车联网中，长期性的资源平衡策略对于保持网络的健康运行和提高整体效率至关重要。因此，未来的研究和解决方案需要更全面地考虑长期资源平衡的因素，以实现持续的网络性能提升。
...
然而，在当前的研究中存在一些差距。主要的关注点集中在任务卸载算法设计上，对任务调度的关注相对较有限，并且这些工作未充分考虑到边缘服务器可能进入休眠状态的情况。在基于MEC的车联网中，涉及各种计算密集型和延迟敏感的任务，每个任务都有不同的资源需求，包括计算和通信资源。这导致了由于车辆位置的地理差异而引起的边缘服务器负载不平衡，进而导致高延迟和能耗增加的问题。虽然当前研究中一些工作考虑了VEC系统中的任务调度算法，但仍需要更多的研究来有效应对这一挑战。
