...
在物联网车辆（IoV）任务调度领域，面临着一系列显著挑战。首先，车辆的动态性增加了任务调度的复杂性，因为车辆的位置、状态和可用性不断变化。其次，波动的网络条件也带来了问题，由于网络连接的不稳定性，任务调度可能受到影响。任务的及时性和复杂性进一步提高了调度的难度，需要在有限时间内完成复杂任务。异构资源的限制意味着不同服务器具有不同的性能和能力，有效地分配不同种类的任务以最大程度利用可用资源变得至关重要。
...
我们的研究目标是通过基于任务特征和预算的调度策略，实现对任务的有效管理，从而提高用户效用、增加边缘服务器收入，并最大化资源利用率。这一系列目标的实现将有助于优化整体服务质量，提高系统经济效益，以适应动态且多变的物联网车辆环境。

在MEC车联网中，车辆的高度移动性和时变性增加了多维资源管理的计算复杂性。为应对这一挑战，需要更智能、实时的资源分配方法。然而，纯优化方法在处理延迟敏感应用和计算复杂性较大的情况时存在困难。

现有方法通常忽视了车辆和服务器之间的连通性，不完全满足现实中的限制条件，可能导致结果过于理想, 无法应用于实际场景。连通性的考虑对任务调度和数据传输至关重要。因此，基于DRL（深度强化学习）的解决方案可能在复杂环境中面临不明确的行动空间激增，需要更好地定义和处理决策问题。

长期资源平衡的考虑被现有方法往往忽略，可能成为阻碍资源利用进一步提高的因素。在车联网中，长期性的资源平衡策略对于保持网络健康运行和提高整体效率至关重要。因此，未来的研究和解决方案需要更全面地考虑长期资源平衡的因素，以实现更好的负载均衡。
