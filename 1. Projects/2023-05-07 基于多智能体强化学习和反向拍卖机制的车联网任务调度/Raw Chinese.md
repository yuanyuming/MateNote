...
人工智能和强化学习可作为快速资源管理的解决方案。[a20][a21][a22][a23]先进的机器学习技术能够解决动态环境下高维度的控制问题。多个代理的深度强化学习技术，将代理部署在边缘服务器上与环境进行分布式交互，降低了与集中式方法相比的控制成本，适用于大规模场景。使用 PPO 算法，使边缘服务器能够根据任务特征和自身资源使用情况自主学习和更新任务调度策略。PPO 算法通过限制策略更新的幅度，避免了过度优化和不稳定的问题，提高了算法的稳定性。在原有的 PPO 方案中添加 LTSM 网络，为策略提供时序信息，考虑任务和服务器资源使用情况的时序变化，用于提取有用特征。

我们借鉴了反向拍卖方法, 构建存在时序和连接限制的多拍卖组任务卸载市场，其中车辆为买方，符合连接限制条件的边缘服务器为卖方。车辆提交任务请求后，服务器根据任务特征和自身状态，通过自身部署的 RL Agent 报价。车辆根据报价选择目标服务器并支付酬劳，类似反向拍卖过程。本文受此启发[^revbid]，考虑到动态车辆网络环境，研究使用深度 RL Agent 报价方法，实现资源合理分配，达到竞争与合作平衡，实现负载均衡，降低车辆卸载成本，增加服务器收益，为基于 MEC 的车联网中对延迟敏感的应用提供支持。
