...
今天，我想向大家介绍一种创新的采购方式——**反向拍卖**。在这个过程中，买方不是设置价格，而是发布招标公告，邀请供应商来竞标。这是一个颠覆传统的模式，因为在反向拍卖中，是供应商来确定价格。

想象一下，我们需要采购一些价格敏感的商品或服务，比如原材料、运输或劳动力。在反向拍卖中，买方会明确规定商品或服务的规格和要求，然后供应商会提交他们的报价，包括价格和交货条件。买方会评估这些报价，选择最合适的获胜者，并与之签订合同。

这样的采购方式有什么好处呢？首先，它可以帮助买方**降低成本**。其次，它可以**提高采购效率**。最后，它还可以**促进供应商之间的竞争**，从而推动市场的健康发展。

反向拍卖不仅仅是一个采购方法，它是一种策略，一种思维方式，它鼓励我们重新思考如何更智能、更高效地进行商业交易。

---


今天，我将向大家介绍强化学习——一种机器学习范式，它让智能体通过与环境的互动来学习如何做出最佳决策。在我们的研究中，我们利用深度强化学习来训练智能体，使其能够在车联网环境中有效地做出决策。

**强化学习**的核心在于智能体能够观察环境状态，执行动作，并从环境中获得奖励。这些奖励信号帮助智能体学习如何在不同的状态下采取最佳动作，以最大化长期回报。

我们的研究进一步扩展到了**深度强化学习**，这是一种结合了深度学习技术的强化学习方法。我们使用神经网络来近似值函数或策略函数，使智能体能够处理更复杂的问题。我们采用了多种深度强化学习算法，如深度Q网络、深度确定性策略梯度，以及异步优势演员-评论家等，来适应不同的问题场景。

在车联网的应用中，我们利用**PPO算法**来训练部署在边缘服务器上的策略网络。这种算法通过限制策略的变化幅度，提高了学习过程的稳定性和准确性。我们的模型结构包括长短期记忆层和全连接层，以更好地处理时间序列数据。

此外，我们还探讨了**多智能体强化学习**，在这个领域中，多个智能体在共享环境中相互作用和学习。我们实现了独立学习和集中式训练分布式执行两种范式，使智能体能够在车联网任务调度中更灵活和适应性地做出决策。

我们的研究展示了强化学习在车联网中的潜力，以及如何通过智能算法来优化资源分配和任务调度。我们相信，这些技术将为未来的车联网带来革命性的变化。


---


在今天的演讲中，我将向大家展示我们如何通过一个基于Python的仿真环境来验证我们的车联网任务调度方法。这个仿真环境模拟了车辆任务调度的整个过程，并提供了一系列评估指标来衡量方法的有效性。

我们的仿真环境受到了PettingZoo多智能体强化学习框架的启发，它为不同的算法提供了一个共同的比较平台。在这个环境中，车辆、服务器和环境生成器协同工作，模拟了任务卸载的动态过程。
...
我们的仿真环境支持Agent Environment Cycle（ACE）模式，这是一个适用于顺序回合制环境的接口，能够处理任何多智能体强化学习算法。在ACE模式下，每个智能体代表一个边缘服务器，它们接收车辆的任务卸载请求，并利用自身资源完成任务卸载，从而获取收益。
...
今天，我要向大家介绍两种强化学习算法：**IPPO（独立PPO）**和**MAPPO**，它们在多智能体系统中的应用日益增多。

首先，让我们来看看**IPPO**，即独立的Proximal Policy Optimization。这是一种强化学习算法，它在多智能体环境中对每个智能体独立地应用PPO算法。在IPPO中，每个智能体都有自己的观察和动作空间，并且它们的策略是独立更新的。这意味着每个智能体都在尝试优化自己的目标，而不考虑其他智能体的策略或行为。这种方法的优点是简单易行，因为它不需要智能体之间的通信或协调。
...
那么，IPPO和MAPPO之间的主要区别是什么呢？简而言之，**IPPO强调独立性**，每个智能体都在自己的世界里学习，而**MAPPO强调协作**，智能体在训练时共享信息，学习如何在一个多智能体的环境中共存和互动。

在实际应用中，选择哪种算法取决于任务的性质和所需的协作水平。如果任务允许或需要智能体独立操作，IPPO可能是一个好选择。而对于需要复杂协作的任务，MAPPO可能更合适。

--


今天，我要向大家介绍我们在车联网中采用的一种先进的技术——多智能体强化学习。在这个框架下，每个边缘服务器都部署了一个智能体，这些智能体使用**PPO（Proximal Policy Optimization）**算法来更新它们的策略。这样做的好处是，服务器可以根据实时的任务需求和市场竞争自适应地调整策略，从而提高它们的收入和为车辆提供的服务效用。

在这个系统中，我们特别关注智能体之间的交互和博弈。每个服务器不仅考虑自己的行为，还考虑其他服务器的行为和可能的影响。为了简化模型，我们假设服务器之间没有额外的通信，并且只进行一次博弈。
...
通过这种方法，我们能够在车联网中实现高效和智能的资源分配和任务调度。我们相信，这将为未来的智能交通系统带来革命性的变化。

---


今天我要向大家介绍我们为模拟车联网场景而构建的公路仿真环境。在这个环境中，多辆车与多个边缘服务器相互作用，车辆会根据服务器的信道状态发出任务卸载请求。我们设计了一个复杂的任务生成系统，其中任务的到达遵循泊松分布，反映了现实世界中任务到达的随机性。

我们的仿真环境包括三种类型的任务：存储密集型、计算密集型和两者兼具的类型，以及长短不同的任务时长。每个任务都有其优先级，这决定了车辆对任务的预算分配，同时也有对CPU和内存的具体需求。

为了更真实地模拟车联网中的资源分配，我们还设定了任务的连接限制，这意味着只有满足特定信道条件的服务器才能接收任务卸载请求。我们的仿真环境的超参数如下：
...
在我们的研究中，我们探讨了任务卸载系统中不同报价策略的性能，并分析了我们提出方法的优势。我们考虑了四种不同的报价策略，它们分别是：
...
我们的目标是通过比较这些策略，来展示我们提出方法的效率和有效性。我们的发现将有助于优化车联网中的资源分配，提高整个系统的性能。

---

在我们的研究中，我们致力于比较不同报价策略在任务卸载系统中的性能。为了全面评估这些策略，我们从多个维度进行了考量，具体包括：

- **负载均衡（Load Balance）**：这一指标衡量的是所有服务器之间资源利用率的一致性。我们通过计算资源占用量与资源总量之比的标准差来评估负载均衡。理想情况下，负载均衡的值越低，意味着服务器资源的利用更加均匀。
...
通过这些维度的评估，我们可以深入了解不同报价策略的优势和局限，进而优化我们的任务卸载系统，提高整体效率和用户满意度。

---


今天我要向大家介绍我们在车联网资源卸载问题上的一项研究。我们评估了两种使用PPO强化学习智能体的方法——**PPO**和**PPO-LSTM**——它们在训练多个智能体以最大化服务器收益方面的收敛性能。

在我们的实验中，我们对这两种智能体进行了长达448,000次迭代的训练，并在每40,000次迭代后进行了策略评估。我们计算了这些策略在10次测试运行中的平均奖励，以此来衡量它们的表现。

我们观察到，虽然**PPO-LSTM策略**的收敛速度稍慢于**PPO策略**，但它在最终收敛后获得的奖励更高，且在训练过程的后期波动更小。这表明PPO-LSTM策略在长期训练中更为稳定，能够更好地捕捉时序信息，做出更合理的决策。

具体来说，PPO-LSTM策略的平均奖励值为1.72×107，比PPO策略的1.68×107高出约2.4%。更重要的是，PPO-LSTM策略的奖励值波动更小，其标准差为1.12×105，比PPO策略的2.65×105低出约57.7%。

这些结果强调了PPO-LSTM策略在处理车联网资源卸载问题时的优势，特别是在需要考虑时间依赖性的复杂决策环境中。

---


在我们的研究中，我们致力于展示我们所提出策略的性能与特点。为此，我们通过仿真实验与其他基准策略进行了对比分析。我们考察了任务到达数目为10、20和40的三种不同场景，并利用五个随机种子进行了十次独立的仿真实验。我们从负载均衡、服务器收益、车辆效用和任务完成率这四个维度对策略进行了评估。
...
在本节中，我们将探讨两种强化学习策略——**MAPPO**和**PPO-LSTM**——在车联网资源卸载问题中的性能对比。我们的目标是评估这些策略在长期训练中的收敛性能。

在我们的实验中，我们进行了240,000次迭代的训练，并对两种策略的奖励进行了评估。我们选择服务器的总体收益作为奖励函数，这直观地反映了任务调度的效果。结果表明，**MAPPO策略**的收敛速度更快，奖励也更高，相比于**PPO-LSTM策略**，在整个训练过程中表现更为优越。这一发现突出了MAPPO中引入的全局价值函数在稳定强化学习训练过程中的重要作用。
...
这些结果说明，MAPPO策略通过全局价值函数的引导，使得边缘服务器的智能体能够在训练过程中学习到其他智能体的信息和环境的信息，从而取得更好的长期收益。这一点在车联网资源卸载问题中尤为重要，因为它要求智能体能够做出快速而准确的决策，以应对动态变化的环境。

---

今天，我要向您介绍我们在车联网资源卸载问题上的最新研究成果。我们的目标是对比两种强化学习策略——**多智能体PPO（MAPPO）**和**PPO-LSTM**——并分析它们在不同任务到达数下的表现。
...
总体来说，无论任务到达数的多少，MAPPO策略都展现了出色的适应性和性能，相较于第3章中的PPO-LTSM策略，它表现得更加优越。我们的研究表明，MAPPO策略通过引入全局价值函数，能够观测到全局信息，包括其他智能体的信息和环境的信息，从而在训练过程中实现更稳定的学习，达到更好的长期收益。

---


今天，我要为大家总结我们在车联网任务调度领域的最新研究成果，并展望未来的研究方向。

**总结：** 我们的研究成功地提出了一种结合多智能体强化学习和反向拍卖机制的方法，这一方法显著提高了车联网中任务的完成率和车辆的效用。我们的方法在效果和稳定性上都优于先前的方法。然而，我们也意识到研究中未考虑的一些因素，如通信可靠性、信道增益的不确定性和参与者的合作性，这些都是我们未来工作的重点。

在实验阶段，我们评估了多种强化学习方法和网络结构，最终选择了PPO-LSTM算法和基于集中式学习分布式部署的MAPPO算法。PPO-LSTM算法通过LSTM捕获时序信息，而MAPPO算法通过全局价值函数观测全局信息，包括其他智能体和环境的信息。这两种方法弥补了其他方法的不足，提供了更高的效率和稳定性。在车联网模拟环境中的对比实验结果表明，MAPPO算法具有明显的优势。

**展望：** 尽管我们的研究取得了进展，但仍有改进的空间。未来，我们计划从以下几个方向进行研究：

- 深入研究通信不可靠性对任务调度的影响，以及在数据丢包和错误情况下的应对策略。
- 探讨信道增益不确定性对车辆连接限制和边缘服务器报价策略的影响，并提出动态调整方法。
- 尝试更多拍卖机制，如二价拍卖、VCG拍卖和时钟拍卖等，以优化任务调度和资源分配效率。
- 测试更多的多智能体强化学习算法，以提高效率和性能，并缩短学习时间。
- 考虑同一时间存在多个任务请求的拍卖过程，研究有效的拍卖和资源分配方法。

我们期望通过这些改进，进一步完善我们的研究，并为车联网任务调度领域的发展做出更大的贡献。
