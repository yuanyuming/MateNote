...
我们使用**RLlib**实现和管理训练过程。RLlib能够并行地从环境中采集数据，将这些数据汇总到一个训练批次中，然后根据选定的算法和损失函数计算梯度，并更新模型参数。这个过程会一直重复，直到达到我们设定的终止条件，比如最大迭代次数、最大训练时间或者最佳性能。
