...
```
@startuml
title Flowchart for Vehicular Network Task Scheduling Model
start
:Vehicle submits task request;
:Task bids for edge servers;
if (Task budget constraint) then (yes)
  if (Server resource constraint) then (yes)
    :Auction allocates task to server;
    :Server executes task;
    :Server receives payment;
    :Vehicle receives output;
    stop
  else (no)
    :Auction rejects task;
    stop
  endif
else (no)
  :Auction rejects task;
...
@startuml
title System Architecture Diagram for Vehicular Network Task Scheduling Model
skinparam componentStyle uml2

component "Vehicle" as V
component "Edge Server" as S
component "Task" as T
component "Auction" as A
